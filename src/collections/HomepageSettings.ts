  ...
  fields: [
    {
      ...
      hooks: {
        beforeChange: [({ data }) => `Акции на главной (${data?.site})`]
      }
    },
    {
      name: 'site',
    ...
    },
    {
      ...
      label: 'Акции на главной странице (порядок — drag & drop)',
      type: 'relationship',
  ...
  hooks: {
    afterChange: [
      async ({ doc, previousDoc, req, operation }) => {
        ...
        const currentPromos = (doc.homepagePromotions || []) as any[]
        const prevPromos = (previousDoc?.homepagePromotions || []) as any[]
